                ...
                <table class="table table-striped table-valign-middle" style="text-align:center;">
                  ...
                  <% 
                        ...
                        ElseIf sortState = "DESC" Then
                            strSQL = "SELECT * FROM HOADONBAN ORDER BY ngay_ban DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
                        End If
                        ...
                        if Not isnull(searchKeyword) and searchKeyword <>"" then
                            strSQL = "SELECT * FROM HOADONBAN WHERE mahoadon_ban LIKE '%" & searchKeyword &  "%' ORDER BY mahoadon_ban OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
                        End If
                  ...
                  <tr>
                    ...
                    <td><%=Result("tongtien_ban")%></td>
                    <td><%=Result("ma_kh")%></td>
                    <td><%=Result("ngay_ban")%></td>
                    <td>
                    ...
                    <td>
                        <% If Result("trang_thai") = 0 Then %>
                          <a href="active1.asp?mahoadon_ban=<%=Result("mahoadon_ban")%>" class="btn btn-success">Active</a>
                        <% Else %>
                          <a href="active1.asp?mahoadon_ban=<%=Result("mahoadon_ban")%>" class="btn btn-danger">Inactive</a>
                        <% End If %>
                    </td>
                    <td>
                        <a href="ctexport.asp?mahoadon_ban=<%=Result("mahoadon_ban")%>" class="btn btn-secondary"><i class="fa-solid fa-eye"></i></a>
                    </td>
                  </tr> 
                  <%
                    Result.MoveNext
